...
public class HeadTracking_Polar : MonoBehaviour
{
    //// UDP通信用
    ...
    void Start()
    {
        ...
        if (calibrationManager == null)
        {
            ...
            if (calibrationManager == null)
            {
                Debug.LogWarning("HeadTracking_Polar: CalibrationManager not found. Calibration will be disabled.");
    ...
    //    // 受信データのスムージング（キャリブレーション済みデータを使用）
    //    currentNoseX = Mathf.SmoothDamp(currentNoseX, calibratedPosition.x, ref velX, angleSmoothTime);
    //    currentNoseY = Mathf.SmoothDamp(currentNoseY, calibratedPosition.y, ref velY, angleSmoothTime);
    //    currentNoseZ = Mathf.SmoothDamp(currentNoseZ, calibratedPosition.z, ref velZ, distanceSmoothTime);
    ...
    void Update()
    {
        ...
        // 🔹 CalibrationManagerから受信データを取得
        Vector3 pos = useCalibration
            ? calibrationManager.GetCalibratedPosition()
            : calibrationManager.GetRawPosition();

        // スムージング
        currentNoseX = Mathf.SmoothDamp(currentNoseX, pos.x, ref velX, angleSmoothTime);
        currentNoseY = Mathf.SmoothDamp(currentNoseY, pos.y, ref velY, angleSmoothTime);
        currentNoseZ = Mathf.SmoothDamp(currentNoseZ, pos.z, ref velZ, distanceSmoothTime);
        ...
        // FOV制御
        float z = Mathf.Max(currentNoseZ, 0.01f);
        float numerator = Mathf.Sqrt(currentNoseX * currentNoseX + adjustedY * adjustedY + z * z);
        currentZoomFactor = Mathf.Clamp(numerator / z, minZoomFactor, maxZoomFactor);
        float targetFOV = baseFOV / currentZoomFactor;
    ...
    //    GUILayout.BeginArea(new Rect(10, 10, 500, 450));
    //    GUILayout.Label("=== Head Tracking Polar (Calibrated) ===");
