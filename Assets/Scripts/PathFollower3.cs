...
public class PathFollower3 : MonoBehaviour
{
    ...
    private volatile bool isPreGestureActive = false; // PREGESTUREが検出されているか
    private volatile string lastReceivedMessage = ""; // 最後に受信したメッセージ
    ...
    void StartUDPReceiver()
    {
        try
        {
            ...
            Debug.Log("PathFollower: UDP Receiver started on port 9997");
        }
    ...
    void ReceiveData()
    {
        IPEndPoint remoteEndPoint = new IPEndPoint(IPAddress.Any, 9997);
    ...
    void Update()
    {
        ...
        // 受信したメッセージに基づいて速度を制御
        if (lastReceivedMessage == "PREGESTURE")
        {
            if (!isPreGestureActive)
            {
                isPreGestureActive = true;
                Debug.Log("PathFollower: PREGESTURE detected - Speed set to 0");
            }
            currentMoveSpeed = stopSpeed;
        }
        else
        {
            if (isPreGestureActive)
            {
                isPreGestureActive = false;
                Debug.Log("PathFollower: PREGESTURE cleared - Speed restored");
            }
            // 通常は最大速度で移動（必要に応じて別の速度制御ロジックに変更可能）
    ...
    //    GUILayout.Label($"Last Message (Port 9997): {lastReceivedMessage}");
    //    GUILayout.Label($"PREGESTURE Active: {isPreGestureActive}");
